...
// Public contract must not expose the internal supplier relation.
const publicContract = contract.slice(contract.indexOf("export function publicProduct"));
assert.doesNotMatch(publicContract, /supplier_id/);
...
assert.doesNotMatch(repository, /SELECT[\s\S]{0,800}supplier_id[\s\S]{0,800}FROM commerce_products/);
...
// Integration gate: Commerce-1 public API is now explicitly wired before asset fallback.
assert.match(worker, /import \{ handlePublicProducts \} from "\.\/src\/commerce\/product-api\.js";/);
assert.match(worker, /if \(url\.pathname === "\/api\/products" && request\.method === "GET"\) return handlePublicProducts\(request, env\);/);
assert.match(worker, /if \(url\.pathname\.startsWith\("\/api\/products\/"\) && request\.method === "GET"\) return handlePublicProducts\(request, env, decodeURIComponent\(url\.pathname\.slice\("\/api\/products\/"\.length\)\)\);/);
assert.match(worker, /if \(url\.pathname\.startsWith\("\/api\/admin\/"\)/);
assert.match(worker, /if \(url\.pathname === "\/api\/inquiries" && request\.method === "POST"\)/);
