...
for (const [name, pattern] of required) {
  if (!pattern.test(auth)) throw new Error(`FAIL: ${name}`);
}
...
if (!/POST\s+\/api\/inquiries|\/api\/inquiries/.test(worker)) {
  throw new Error("FAIL: public inquiry route could not be located for regression review");
}
...
console.log("NOTE: this is a static contract check; live Preview/authentication is not executed by this script.");
