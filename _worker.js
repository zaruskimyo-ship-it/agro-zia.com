...
function inquiryEmailText({ requestNumber, createdAt, language, product, company, specification, quantity, destination, timing, email, phone, message, attachment }) {
  return [
    "AGRO-ZIA BUSINESS INQUIRY", "", `Request Reference: ${requestNumber}`, `Request Date & Time: ${createdAt}`, `Language: ${language}`, "",
    `Company: ${company || ""}`, `Country: ${destination || ""}`, `Product / Interest: ${product || ""}`, `Required specification: ${specification || ""}`,
    `Quantity: ${quantity || ""}`, `Preferred timing: ${timing || ""}`, `Email: ${email || ""}`, `Phone: ${phone || ""}`, "", message || "", "",
...
async function sendInquiryEmail(env, data) {
  ...
  const result = await env.EMAIL.send({ to: INQUIRY_EMAIL_TO, from: INQUIRY_EMAIL_FROM, subject: `Agro-Zia Business Inquiry ${data.requestNumber}`, text: inquiryEmailText(data), replyTo: data.email || undefined });
...
function inquiryTelegramText({ requestNumber, createdAt, language, product, company, specification, quantity, destination, timing, email, phone, message, attachment }) {
  return [
    "AGRO-ZIA BUSINESS INQUIRY", "", `Reference: ${requestNumber}`, `Date: ${createdAt}`, `Language: ${language}`, `Company: ${company || ""}`,
    `Country: ${destination || ""}`, `Product / Interest: ${product || ""}`, `Required specification: ${specification || ""}`, `Quantity: ${quantity || ""}`,
    `Preferred timing: ${timing || ""}`, `Email: ${email || ""}`, `Phone: ${phone || ""}`, "", message || "", "",
...
async function sendInquiryTelegram(env, data) {
  ...
  if (hasAttachment) {
    ...
    if (!attachmentObject) return { status: "failed" };
    ...
    response = await fetch(apiBase + "/sendMessage", {
      ...
      body: JSON.stringify({ chat_id: env.TELEGRAM_CHAT_ID_V2, text: telegramText(inquiryTelegramText(data), 4000), disable_web_page_preview: true }),
    });
  }
  const result = await response.json().catch(() => null);
  console.log("Telegram API response received", { operation, status: response.status, ok: Boolean(result?.ok), error_code: result?.error_code || null });
...
async function createInquiry(request, env) {
  ...
  for (const field of Object.keys(MAX_LENGTHS)) if (exceedsMaxLength(body[field], MAX_LENGTHS[field])) return json({ error: "field_too_long", field, max_length: MAX_LENGTHS[field] }, 400);
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    ...
    try {
      ...
      if (attachment) {
        ...
        storedObject = await env.AGROZIA_ATTACHMENTS.put(attachmentKey, attachment.stream(), { httpMetadata: { contentType: attachment.type || "application/octet-stream", contentDisposition: `attachment; filename="${safeName}"` }, customMetadata: { request_number: requestNumber, original_name: safeName } });
      }
      await env.AGROZIA_DB.prepare(`INSERT INTO inquiries (request_number, created_at, language, product, company, specification, quantity, destination, timing, email, phone, message, status, attachment_key, attachment_name, attachment_type, attachment_size, attachment_etag) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'received', ?, ?, ?, ?, ?)`).bind(requestNumber, createdAt, language, product, company || null, specification || null, quantity || null, destination || null, timing || null, email || null, phone || null, message || null, attachmentKey, attachment?.name || null, attachment?.type || null, attachment?.size || null, storedObject?.httpEtag || storedObject?.etag || null).run();
      let emailNotification = { status: "unconfigured" };
      let emailNotificationError = null;
      try { emailNotification = await sendInquiryEmail(env, { requestNumber, createdAt, language, product, company, specification, quantity, destination, timing, email, phone, message, attachmentKey, attachmentName: attachment?.name || null, attachmentType: attachment?.type || null, attachment: attachment ? { name: attachment.name, type: attachment.type, size: attachment.size } : null }); }
      catch (emailError) { emailNotificationError = safeEmailErrorCode(emailError); emailNotification = { status: "failed" }; }
      let telegramNotification = { status: "unconfigured" };
      try { telegramNotification = await sendInquiryTelegram(env, { requestNumber, createdAt, language, product, company, specification, quantity, destination, timing, email, phone, message, attachmentKey, attachmentName: attachment?.name || null, attachmentType: attachment?.type || null, attachment: attachment ? { name: attachment.name, type: attachment.type, size: attachment.size } : null }); }
      catch (_) { telegramNotification = { status: "failed" }; }
      return json({ ok: true, persisted: true, request_number: requestNumber, status: "received", created_at: createdAt, attachment: attachment ? { name: attachment.name, type: attachment.type, size: attachment.size } : null, email_notification: emailNotification.status, email_notification_error: emailNotificationError, telegram_notification: telegramNotification.status }, 201);
    } catch (error) {
      if (attachmentKey && env.AGROZIA_ATTACHMENTS) try { await env.AGROZIA_ATTACHMENTS.delete(attachmentKey); } catch (_) {}
...
<div class="contact-grid" id="canonical-business-inquiry">
  <div><p class="eyebrow">BUSINESS INQUIRY</p><h2 data-i18n="contact_title"></h2><p class="section-text" data-i18n="contact_text"></p><p><strong>info@agro-zia.com</strong><br><strong>export@agro-zia.com</strong></p></div>
  <form id="canonical-inquiry-form" novalidate><div class="form-grid">
    <label><span data-i18n="name"></span><input name="company_contact_name" autocomplete="name"></label><label><span data-i18n="company"></span><input name="company" autocomplete="organization"></label>
    <label><span data-i18n="country"></span><input name="destination" autocomplete="country-name"></label><label><span data-i18n="email"></span><input type="email" name="email" autocomplete="email"></label>
    <label><span data-i18n="phone"></span><input name="phone" autocomplete="tel"></label><label><span data-i18n="interest"></span><select name="product"><option value="Product">Product</option><option value="Engineering">Engineering</option><option value="Project">Project</option><option value="Trade">Trade</option></select></label>
    <label class="full"><span data-i18n="specification_label">Required specification</span><input name="specification"></label><label><span data-i18n="quantity_label">Quantity</span><input name="quantity"></label><label><span data-i18n="timing_label">Preferred timing</span><input name="timing"></label>
    <label class="full"><span data-i18n="message"></span><textarea name="message"></textarea></label>
  </div><button class="btn" type="submit" data-i18n="submit"></button><p class="section-text" data-i18n="form_note"></p><div class="result" id="inquiry-result" aria-live="polite"></div></form>
</div>`;

function transformMultilingualPreview(response) { return new HTMLRewriter().on("#contact .contact-grid", { element(element) { element.replace(MULTILINGUAL_INQUIRY_FORM, { html: true }); } }).transform(response); }

export default {
  async fetch(request, env) {
    const url = new URL(request.url);
    if (url.pathname.startsWith("/api/admin/")) { const adminResponse = await dispatchAdminRoute(request, env); if (adminResponse) return adminResponse; }
    ...
    if (url.pathname === "/api/rfqs" && request.method === "POST") return handlePublicRfqs(request, env);
    if (url.pathname === "/api/suppliers" && request.method === "GET") return json({ error: "not_found" }, 404);
    if (url.pathname.startsWith("/api/suppliers/") && request.method === "GET") return handlePublicSuppliers(request, env, decodeURIComponent(url.pathname.slice("/api/suppliers/".length)));
    if (url.pathname === "/api/inquiries" && request.method === "POST") return createInquiry(request, env);
    if (url.pathname === "/api/health" && request.method === "GET") return json({ ok: true, service: "agro-zia-inquiry-api", d1_bound: Boolean(env.AGROZIA_DB), attachment_storage_bound: Boolean(env.AGROZIA_ATTACHMENTS), email_bound: Boolean(env.EMAIL) });
