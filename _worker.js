...
async function renderMultilingualPreview(request, env) {
  ...
  if (contactStart < 0 || contactEnd < 0) return new Response(html, { status: asset.status, headers: asset.headers });
  ...
  if (formStart < 0 || formEnd < 0) return new Response(html, { status: asset.status, headers: asset.headers });
