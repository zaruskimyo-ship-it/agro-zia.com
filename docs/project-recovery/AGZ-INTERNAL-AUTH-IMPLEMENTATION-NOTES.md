...
## Current status
Repository-side security primitives are prepared on `feat/stage-12-auth-internal`. The Worker router is intentionally not switched to these primitives yet because the exact routing section must be integrated without replacing unrelated code.
...
- Session material contains cryptographically random nonce material.
...
- Login abuse protection must be bounded and must not depend on process-local memory as the sole production control.

## Cloudflare note
Workers Web Crypto supports HMAC, SHA-256, `crypto.getRandomValues()`, and timing-safe equality operations. Sensitive values should be stored as Worker Secrets, not source code or ordinary variables.
