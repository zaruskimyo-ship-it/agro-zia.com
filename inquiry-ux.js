...
(() => {
  ...
  const TYPES = new Set(['application/pdf','application/msword','application/vnd.openxmlformats-officedocument.wordprocessingml.document','text/plain','image/jpeg','image/jpg','image/png','image/webp']);
  ...
  function installSharedNavigation() {
    ...
    const inquiryMenu = document.querySelector('[data-inquiry-menu]');
    if (inquiryMenu) inquiryMenu.setAttribute('data-menu', 'true');
    const head = document.head;
    if (!head) return;
    ['assets/floating-nav.css','assets/bottom-actions.css'].forEach((href) => {
      if (!document.querySelector(`link[href="${href}"]`)) {
        const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = href; head.appendChild(link);
      }
    });
    ['assets/floating-nav.js','assets/bottom-actions.js'].forEach((src) => {
  ...
  function install() {
    ...
    if (fileInput) fileInput.addEventListener('change', () => { const file = fileInput.files?.[0]; if (!file) return; if ((!TYPES.has(file.type) && file.type) || !EXTENSIONS.test(file.name || '')) { fileInput.value=''; setResult(t.invalidType,true); return; } if (file.size > MAX_BYTES) { fileInput.value=''; setResult(t.invalidSize,true); } });
    form.addEventListener('submit', async (event) => {
      event.preventDefault(); if (button.disabled) return; button.disabled=true; const originalLabel=button.textContent; button.textContent=t.sending; result.className='result'; result.textContent='';
      try {
        const source=new FormData(form); const body=new FormData(); for(const [key,value] of source.entries()) if(key!=='attachment' && typeof value==='string') body.append(key,value); body.set('language',lang);
        const attachment=fileInput?.files?.[0]; if(attachment){if((!TYPES.has(attachment.type)&&attachment.type)||!EXTENSIONS.test(attachment.name||''))throw new Error(t.invalidType);if(attachment.size>MAX_BYTES)throw new Error(t.invalidSize);body.append('attachment',attachment,attachment.name);}
        const response=await fetch('/api/inquiries',{method:'POST',body,credentials:'same-origin'}); const payload=await response.json().catch(()=>({})); if(!response.ok||payload.ok!==true||payload.persisted!==true||!payload.request_number)throw new Error(payload.error||`HTTP ${response.status}`);
        const reference=escapeHtml(payload.request_number); const attachmentName=escapeHtml(payload.attachment?.name||''); const attachmentNote=attachmentName?`<div>Attachment: ${attachmentName}</div>`:''; const emailNote=payload.email_notification==='sent'?t.emailSent:t.emailFailed;
        result.className='result show'; result.innerHTML=`<strong>${t.success}</strong><div style="margin-top:6px">${t.reference}: <strong>${reference}</strong></div>${attachmentNote}<div style="margin-top:6px">${t.saved}</div><div style="margin-top:6px">${emailNote}</div>`; form.reset();
      } catch(error){ setResult(t.error+(error?.message||'request_failed'),true); } finally { button.disabled=false; button.textContent=originalLabel; }
    });
  }
