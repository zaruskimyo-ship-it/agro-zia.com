const COOKIE = "agz_customer_session";
...
function secret(env) {
  return typeof env.CUSTOMER_SESSION_SECRET === "string" && env.CUSTOMER_SESSION_SECRET.length >= 32
    ? env.CUSTOMER_SESSION_SECRET
    : null;
}

export async function hashLoginCode(code, challengeId) {
...
export async function createCustomerSession(env, accountId, now = Date.now()) {
  const key = secret(env);
  if (!key || !accountId) return null;
  const payload = `${accountId}.${Math.floor(now / 1000) + TTL_SECONDS}`;
  const signature = b64url(await hmac(key, payload));
  return `${payload}.${signature}`;
}

export async function readCustomerSession(request, env, now = Date.now()) {
  const key = secret(env);
  if (!key) return null;
  ...
  if (!accountId || !/^\d+$/.test(expires) || Number(expires) < Math.floor(now / 1000)) return null;
  ...
  return diff === 0 ? { accountId, expiresAt: Number(expires) } : null;
}

export function customerSessionCookie(token, maxAge = TTL_SECONDS) {
